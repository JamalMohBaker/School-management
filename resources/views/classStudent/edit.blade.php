...
<form id="add_Classroom_Student">
    ...
    <div class="row gy-3">
        <div class="col-xl-12">
            {{-- <div>{{$class_student->user_id}}</div>
            <div>{{$class_student->classroom_id}}</div> --}}
            ...
            <select class="js-example-basic-single" name="user" id="user">
                ...
                <option value="{{$user->id}}" @selected($user->id == $class_student->user_id)> {{$user->first_name }} </option>
                @endforeach
            </select>
            ...
            <select class="js-example-basic-single" name="classroom" id="classroom">
                ...
                <option value="{{$classroom->id}}" @selected($classroom->id == $class_student->classroom_id)>{{$classroom->grade->name}} {{$classroom->name }} </option>
                @endforeach
            </select>


            <button type="button" onclick="performUpdate('{{ $class_student->classroom_id }}', '{{ $class_student->user_id }}')" class="btn btn-primary mt-3 w-100"> 
...
<script>
    ...
    function performUpdate(classroom_id, user_id) {
        
                axios.put(`/class_students/${classroom_id}/${user_id}`, {
