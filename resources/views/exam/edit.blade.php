...
<form class="row g-3 mt-0" method="post" action="{{route('exams.update',$exam->id)}}">
    @csrf
    @method('PUT')
    @if(session('success'))
    ...
    <div class="col-md-6">
        ...
        <input type="text" value="{{ old('title' , $exam->title ) }}" name="title" class="form-control" id="title"
            >
    </div>
    <div class="col-md-6">
        ...
        <select class="js-example-basic-single" name="subject_teacher_classrooms" id="subject_teacher_classrooms">
            ...
            <option value="{{$class_student->id}}" {{ old('subject_teacher_classrooms', 
            $exam->subject_teacher_classroom_id)==$class_student->id ? 'selected': '' }}>
                 {{$class_student->classroom->grade->name}}
                {{$class_student->classroom->name}} Class {{$class_student->subject->name}}
                Language with Teacher {{$class_student->user->first_name}}
    ...
    <div class="col-md-6">
        ...
        <input type="text" value="{{ old('time_duration', $exam->duration_minutes) }}" name="time_duration" class="form-control"
            id="time_duration" >
    </div>
    <div class="col-md-6">
    ...
    <div class="col-md-6">
        ...
        <input type="time" value="{{ old('time_open', $exam->start_at) }}" name="time_open" class="form-control"
            id="time_open">
    </div>
    <div class="col-md-6">
        <label for="time_close" class="form-label"> Time Closed </label>
        <input type="time" value="{{ old('time_close', $exam->end_at) }}" name="time_close" class="form-control"
            id="time_close">
    </div>

    <div class="col-md-3">
        <label for="question_num" class="form-label"> How Many Questions For Students? </label>
        <input type="text" value="{{ old('question_num', $exam->question_num) }}" name="question_num" class="form-control"
            id="question_num" placeholder="10">
    </div>
    <div class="col-md-3">
        <label for="score" class="form-label"> Score Per Question </label>
        <input type="text" value="{{ old('score', $exam->score) }}" name="score" class="form-control" id="score"
            placeholder="1">
    </div>
    <div class="col-md-3">
        <label for="final_score" class="form-label"> Final Score </label>
        <input type="text" value="{{ old('final_score', $exam->final_score) }}" name="final_score" class="form-control" id="final_score"
            placeholder="20">
    </div>
    <div class="col-md-3">
        <label for="show_score" class="form-label"> Show Score </label>
        <select class="js-example-basic-single" name="show_score" id="show_score">
            <option value="1" {{ $exam->show_score == 1 ? 'selected' : '' }}>Yes</option>
            <option value="0" {{ $exam->show_score == 0 ? 'selected' : '' }}>No</option>
        </select>
        
    </div>
    <button type="submit" class="btn btn-primary mt-3 w-100">Update <i class="fa-solid fa-circle-check"></i></button>
