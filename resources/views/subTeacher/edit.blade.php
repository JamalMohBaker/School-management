...
<form id="add_Subject_teacher">
    ...
    <div class="row gy-3">
        <div class="col-xl-12">
            ...
            <input type="text" class="form-control" id="description" value="{{$sub_teacher->description}}" >
            ...
            <select class="js-example-basic-single" name="classroom" id="classroom">
                ...
                <option value="{{$classroom->id}}" @selected($classroom->id == $sub_teacher->classroom->id)>{{$classroom->grade->name}} {{$classroom->name }} </option>
                @endforeach
            </select>
            ...
            <select class="js-example-basic-single" name="subject" id="subject">
                ...
                <option value="{{$subject->id}}" @selected($subject->id == $sub_teacher->subject->id)> {{$subject->name }} that code {{$subject->code}} </option>
                @endforeach
            </select>
            ...
            <select class="js-example-basic-single" name="teacher" id="teacher">
                ...
                <option value="{{$teacher->id}}" @selected($teacher->id == $sub_teacher->user->id)> {{$teacher->first_name }} </option>
                @endforeach
            </select>
            <button type="button" onclick="performUpdate({{ $sub_teacher->id }})" class="btn btn-primary mt-3 w-100"> {{__('dashboard.save')}} </button>
...
<script>
    ...
    function performUpdate() {
                let id = {{ $sub_teacher->id }};
                axios.put(`/sub_teachers/${id}`, {
